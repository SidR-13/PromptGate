...
**`reasons` ordering — deterministic but not a severity ranking:** the list is always built in the fixed source order `score → blocked → locale_checks` (three sequential `if` statements), so the same run always produces reasons in the same order. This order is *not* an intentional priority ranking — it happens to match the order fields were listed in the original spec, not a judgment that moderation failures matter more than low scores or vice versa. **Step 8's CI/PR comment should render `reasons` as a flat, unordered bullet list** — not "primary reason: X" or any framing that implies the first item is the most severe, since no such ranking was ever decided.
